document.addEventListener('DOMContentLoaded', () => {
    ...
    // Глобальная переменная для хранения данных из БД
    window.apiCarsData = null;

    // Fetch запрос для получения машин из базы данных (Django)
    fetch('/api/cars/')
        .then(response => response.json())
        .then(data => {
            window.apiCarsData = data;
        })
        .catch(error => console.error('Ошибка при загрузке автомобилей:', error));
    ...
    function fillCarModal(slide) {
        ...
        // Берем данные из переменной, которую мы заполнили через fetch
        const car = window.apiCarsData ? window.apiCarsData[modelKey] : null;
        if (!car) {
            console.error("Данные для модели", modelKey, "не найдены в базе данных.");
            return;
        }
        ...
        techList.innerHTML = '';
        if (car.tech && Array.isArray(car.tech)) {
